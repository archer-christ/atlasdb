...
.. list-table::
    ...
    *    - |fixed|
         - RemoteLockService clients will no longer silently retry on connection failures, and will also no longer retry exceptions other than 503s with a specified 'Retry-After' header (inclusive of NotCurrentLeaderExceptions).
    ...
    *    - |fixed|
         - AtlasDbHttpClients no longer retry the same node three times in succession when dealing with a NotCurrentLeaderException.
